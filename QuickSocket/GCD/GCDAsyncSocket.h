...
#if !TARGET_OS_IPHONE
extern NSString *const GCDAsyncSocketSSLDiffieHelanParameters;
#endif
 ...
 * Local ports do NOT need to match remote ports. In fact, they aost never do.
 ...
 * a request/response was built up in memory. Copying this data adds an unwanted/unneeded oveead.
 ...
 *
 * - GCDAsyncSocketSSLDiffieHelanParameters (Mac OS X only)
 *     The value must be of type NSData.
 *     See Apple's documentation for SSLSetDiffieHelanParams.
 * 
 ...
 *         result = [self someInternaethodToBeRunOnlyOnSocketQueue];
 *     }
