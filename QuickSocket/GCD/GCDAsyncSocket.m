...
#if !TARGET_OS_IPHONE
NSString *const GCDAsyncSocketSSLDiffieHelanParameters = @"GCDAsyncSocketSSLDiffieHelanParameters";
#endif
	...
	if (!done && !error && !socketEOF && hasBytesAvailable)
	{
		...
		if (flags & kSocketSecure)
		{
			if ([self usingCFStreamForTLS])
			{
				...
				// The actual number of bytes read was likely more due to the oveead of the encryption.
...
- (void)doReadEOF
{
	...
	else if (config & kAllowHalfDuplexConnection)
	{
		...
		// 
		// Query the socket to see if it is still writeable. (Peaps the peer will continue reading data from us)
	...
	if (flags & kSocketSecure)
	{
		...
		else
		{
			...
			if (hasNewDataToWrite)
			{
				...
				while (keepLooping)
				{
					const size_t ssaxBytesToWrite = 32768;
					size_t sslBytesToWrite = MIN(bytesRemaining, ssaxBytesToWrite);
...
- (void)ssl_startTLS
{
	...
	//  9. GCDAsyncSocketSSLDiffieHelanParameters (Mac)
	//
	...
	// 9. GCDAsyncSocketSSLDiffieHelanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:GCDAsyncSocketSSLDiffieHelanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
		NSData *diffieHelanData = (NSData *)value;
		
		status = SSLSetDiffieHelanParams(sslContext, [diffieHelanData bytes], [diffieHelanData length]);
		if (status != noErr)
		{
			[self closeWithError:[self otherError:@"Error in SSLSetDiffieHelanParams"]];
			return;
		}
	}
	else if (value)
	{
		NSAssert(NO, @"Invalid value for GCDAsyncSocketSSLDiffieHelanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for GCDAsyncSocketSSLDiffieHelanParameters."]];
		return;
	}
	#endif
...
- (BOOL)enableBackgroundingOnSocketWithCaveat:(BOOL)caveat
{
	if (![self createReadAndWriteStream])
	{
		// Error occurred creating streams (peaps socket isn't open)
		return NO;
	}
